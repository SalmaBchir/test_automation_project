...
from utils.users import InvalidUserData
from utils.validation_messages.forgot_password_page_messages import ForgotPasswordPageMessages


class TestForgotPasswordInvalidEmail(BaseTest):
    @pytest.mark.parametrize("email", InvalidUserData.generate_invalid_email())
    def test_forgot_password_invalid(self, email):
        """Verify that the system rejects password reset requests with invalid email formats."""
        ...
        expected_error = ForgotPasswordPageMessages.INVALID_EMAIL
